...
# Does this work --- need to test further

class GmmMgfStd(GmmBase):
    ...
    def __init__(self, mgf, bounds, num_moments, x):
        ...
        self.second_moment = Partial(grad(grad(mgf)), 0.0)
        self.inv_scale = lambda theta: jnp.power(self.second_moment(theta) - self.first_moment(theta)**2, -0.5)
        self.std_moment_functions = utils.standardized_moment_functions(mgf, num_moments)
        # Data
    ...
    def loss_components(self, theta):
        std_moments = jnp.array([self.std_moment_functions[i](theta) for i in range(0, self.num_moments)])
        mu1 = self.first_moment(theta)
        mu_poly = jnp.array([mu1**k for k in range(1, self.num_moments+1)])
        inv_scale = self.inv_scale(theta)
        inv_scale_poly = jnp.array([inv_scale**k for k in range(1, self.num_moments+1)])
        return (jnp.matmul(self.a, mu_poly) + self.moments_1N[1:]) * inv_scale_poly - std_moments
    ...
    def loss_components_expanded(self, theta):
        ...
        # x_m_mu1 = self.x - mu1
        # return jnp.column_stack([x_m_mu1**(k+1) - std_moments[k] for k in range(0, self.num_moments)])
        return (jnp.matmul(self.A, mu_poly) + self.x_poly_1N[:,1:]) * inv_scale_poly.reshape((1, self.num_moments)) - std_moments.reshape((1, self.num_moments))
