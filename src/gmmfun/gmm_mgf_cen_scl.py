...
class GmmMgfCenScl(GmmBase):
    ...
    def loss_components(self, theta):
        ...
        return (jnp.matmul(self.a, mu_poly) + self.moments_1N[1:] - central_moments) * inv_scale_poly
    ...
    def loss_components_expanded(self, theta):
        ...
        inv_scale = self.inv_scale(theta)
        inv_scale_poly = jnp.array([inv_scale**k for k in range(1, self.num_moments+1)])
        return (jnp.matmul(self.A, mu_poly) + self.x_poly_1N[:,1:] - central_moments.reshape((1, self.num_moments))) * inv_scale_poly.reshape((1, self.num_moments))
